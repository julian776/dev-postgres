...
- Writes hidden inside CTEs (`WITH ... AS (INSERT INTO ...)`)
- Dynamic SQL in `DO $$ ... $$` blocks
- Function calls that perform writes (`SELECT my_write_function()`)
- Queries constructed with string concatenation
...
The auto-LIMIT feature (appending `LIMIT N` to unbounded SELECTs) uses simple regex detection. Multi-statement queries or complex subqueries may not be correctly handled. Add explicit LIMIT clauses for predictable behavior.
...
Although `PGPASSWORD` is used (and unset immediately after), it may briefly appear in `/proc` or `ps` output. For high-security environments, use a `.pgpass` file or `PGPASSFILE` environment variable instead. See security.md for details.
